...
function begin() {
  var newLine = lines[Math.floor(Math.random() * lines.length)] + punctuation[Math.floor(Math.random() * punctuation.length)];
  newLine = document.title;
  newBg();
}
...
function newText() {
  ...
  var paragraph = document.getElementById("poem");
  var text = document.createTextNode(newLine);
  document.title = newLine;
